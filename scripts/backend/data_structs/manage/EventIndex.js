define([
  ...
  var EventIndex = {
    ...
    addListener: function(subscriber, target, op, callback) {
      // create entries in listener map if nonexistent
      ...
      // create entry in subscriber map if nonexistent
      if (!this._subscriberMap[subscriber]) {
        this._subscriberMap[subscriber] = [];
      }

      // populate entry in listener map
      ...
      // populate entry in subscriber map
